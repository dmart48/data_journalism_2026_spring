...
**Answer** 7314 obs. of 29 Variables
...
In the space below, write the datatype for the "EIN" column and the "ASSET_AMT" column (it's in light gray, offset by \< \>). **Answer** EIN = chr     ASSET_AMT = dbl
...
**Answer**  This block is taking all the cities mentioned in the table and organizing them in alphabetical order. Then it adds the number of churches designated in each each city. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** In this codeblock we added the arrange command in descending order which orders the cities in order of most churches to least. 
...
**Answer** This time, we addeed the zip code category to the list so that the list is ordered by zip codes with the most churches. The city is still identified but the zip codes make the list more specific. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** This codeblock is similar to previous ones because we are still using the "group_by" command and the "arrange" command. However, this block is for the megachurch table and we are grouping by denomination and ordering by size of the congregation. 
...
1.  From megachurches, which states have the most megachurches?

2.  From megachurches, which denominations have the most amount of churches? 

3.  From md_churches, do any ICO's repeat? Is there anyone that is in care of multiple churches?
