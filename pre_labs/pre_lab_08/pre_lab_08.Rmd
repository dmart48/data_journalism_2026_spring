...
#The first section is turning the "adherents_as_percent_of_population" column to a numeric column instead of a character column. The second section takes the top 10 states with the most congregations and displays them in descending order. 
...
#This code just creates a blank space. 
...
#Now we see the bar chart of the top 10 states with the most congregations, but they are in alphabetical order. 
...
#Now we see the bar chart in order of least congregations to most going left to right from the top 10 list. 
...
#Now we see the bar chart flipped horizontally which makes the state names much easier to read. 
...
#This code removed the gray background from the chart. 
...
# This code made the chart look a little better by changing the background, the grid, and the x axis. This makes it easier to read as well. 
...
#This code changed the names of the x and y axis as well as added a title in bold font. It also added a source attribution caption at the bottom. 
...
# We see different types of religious data for a fifteen year interval starting at 1965. 
...
# We see a line plot showing the declining number of priests over the last 60 years. 
...
#Now we see dots at the intervals the data was collected. 
...
#Now we see a title, cleaned axis headers, and a source caption. 
...
#We see a dot plot showing a generally positive correlation between adherents as a percent of population and diversity index value. There is one outlier that has a very high percent of adherents and a lower diversity index value. Also, as both values rise, the correlation gets weaker. 
...
Now we get cleaned axis headers, a title, and a line of best fit. 
