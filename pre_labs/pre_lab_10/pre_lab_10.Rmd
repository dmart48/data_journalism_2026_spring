...
In this dataset we have zipcodes, zip names, shape columns, and a geometry colum. 
...
#We get a map of Maryland with all the zipcodes outlined.
...
# "Counties" gives us a dataset of all counties in the US with a unique ID and geometry data. 
...
#Now we narrowed down the counties to just Maryland counties. 
...
#We get a map of Maryland with the counties outlined. The Bay is gone becuase the county lines extend into the water. 
...
#Now we get a map of Maryland with the counties on it, then the zip codes on top. 
...
#This code takes the PG County from md_counties and also takes the zips from md_zips to create a dataset that will give us the zipcodes in PG county. 
...
# This code gives us 2120 churches in PG county. 
...
#This code gives us the number of churches in each zip code in PG county and the geometry data for those churches.
...
#THe map of PG county gives us a color coded zip map based on the number of churches in each zip. We see more churches around DC and in the center of the county, and much less on the outer limits of the county. We also see no churches in places like Joint Base Andrews and a zipcode in southern Virginia. 
...
#This code gives us Maryland county population data and religion data. 
...
#This code gave us geometry for the md_county_population dataset. 
...
# This code cleaned up the population data by removing the "Maryland" from the county column. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
# This code joined the two datas with left join so that we get all the columns. 
...
# Now we added a rate column that equals the number of congregations per 10,000 people and arranged the dataset in descending order. 
...
THis code created a map with all the Maryland counties and colored them by the rate of churches in the county. There are labels for each county. Baltimore city has no data. 
...
#Now we have a similar map without the labels and with a different color scheme. 
...
#In this map, the difference betwen some counties are more defined while the difference between others are less defined. Rather than the whole middle of Maryland being black, there is now some difference with the purple. But we lose some distinction in northern Maryland. 
