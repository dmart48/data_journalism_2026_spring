...
**Answer** 
#Chi Alpha is a campus ministy organization. On their website, they do not identify University of Maryland as a location of a group. The most similarly named organization on campus is the lambda chi alpha fraternity. 
...
```{r}
md_churches |>
  filter(str_detect(CITY, "ROCKVILLE"))
```

**Answer**
#I chose to search Rockville because my hometown is Rockville, so I wanted to see all the churches there. 
...
**Answer:**
#The date column is now in a more universal format. 
...
**Answer**
#The month column is taking the transactions from each month and grouping them together by month and year, so every transaction for august 2024 is now totaled under 2024-08-01.
...
**Answer**
#This block of code is taking the total catholic population of each year then mathing it up to the number of parishes in that year and dividing to find the number of catholics per parish. The code also creates a new column with this data. 
...
**Answer**
#The table is now in descending order of catholics per parish.
...
**Answer**
#The data shows that the catholics per parish is increasing over time but the percent of parishes with no priest is also increasing. This likely means that parishes are consolidating as there becomes less of them, but the number of priests is declining faster which means there are still less priests to "go around" so to speak. 
...
**Answer**
# str_to_lower turns all letter lower case. str_to_title turns the first letter of each word upper case. 
...
**Answer**
# This code block finds mentions of denominations in the titles of churches in the payee_upper section and using them to identify their denominations based on the key word in the name of the payee. 
...
**Answer**
# Generally, this answer makes sense. The denominations that I would expect to be at the top are at the top. However, it is not necessarily a good tool because a large chunk of the data falls under N/A due to the restraints of the code even though they would identify themselves as a denomination. 
...
1.  Which city has the most expenditures?

2.  Which months see the most expednitures per year? Which years?

3.  What are the most common "purposes" and/or categories of expenditures? 
